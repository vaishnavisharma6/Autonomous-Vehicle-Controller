...
class Controller2D(object):
    ...
    def update_controls(self):
        ...
        if self._start_control_loop:
            ...
            k_e = 0.3
            k_v = 10
            yaw_path = np.arctan2(waypoints[-1][1] - waypoints[0][1], waypoints[0][1] - waypoints[-1][0] - waypoints[0][0])
            yaw_diff = yaw_path - yaw
            if yaw_diff > np.pi:
                yaw_diff = yaw_diff - self._2pi
            if yaw_diff < -1 * np.pi:
                yaw_diff = yaw_diff + self._2pi
            ...
            yaw_modify = yaw_diff - crosstrack_yaw
            if yaw_modify > np.pi:
                yaw_modify = yaw_modify - self._2pi
            if yaw_modify < -1 * np.pi:
                yaw_modify = yaw_modify + self._2pi
            if yaw_modify < 0:
                crosstrack_error = -1 * abs(crosstrack_error)
            else:
                crosstrack_error = abs(crosstrack_error)
            psi_crosstrack = np.arctan(ke * crosstrack_error / (kv + v))
            sigma = yaw_diff + psi_crosstrack
            if sigma > np.pi:
                steer_output = sigma - self._2pi
            if sigma < -1 * np.pi:
                steer_output = sigma + self._2pi
            print(crosstrack_error,yaw_diff,psi_crosstrack)
